...
impl BnpConverter<'_> {
    pub fn handle_actorinfo(&self) -> Result<()> {
        ...
        if path.exists() {
            ...
            if let Some(MergeableResource::ActorInfo(mut info)) = actorinfo.as_mergeable().cloned()
            {
                info.0.extend(
                    diff.into_iter()
                        ...
                        .collect::<Result<BTreeMap<u32, Byml>>>()?
                        .into_iter(),
                );
                fs::write(
                    self.path
                    ...
                    compress(info.into_binary(self.core.settings().current_mode.into())),
                )?;
            }
        }
        Ok(())
